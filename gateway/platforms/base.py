...
class MessageEvent:
    ...
    def get_command(self) -> Optional[str]:
        ...
        # Telegram groups append @bot_username to commands, e.g. /approve@MyBot
        parts = self.text.split(maxsplit=1)
        return parts[0][1:].split("@")[0].lower() if parts else None
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            if event.message_type == MessageType.PHOTO:
                print(
                    f"[{self.name}] 🖼️ Queuing photo follow-up for session {session_key} without interrupt"
                )
            ...
            print(
                f"[{self.name}] ⚡ New message while session {session_key} is active - triggering interrupt"
            )
            self._pending_messages[session_key] = event
                ...
                for media_path, is_voice in media_files:
                    ...
                    try:
                        ...
                        if not media_result.success:
                            print(
                                f"[{self.name}] Failed to send media ({ext}): {media_result.error}"
                            )
